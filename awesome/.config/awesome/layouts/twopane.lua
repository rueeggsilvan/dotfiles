  ...
  capi.mousegrabber.run(function(_mouse)
    ...
    for _, v in ipairs(_mouse.buttons) do
      if v then
        ...
        c.screen.selected_tag.master_width_factor = math.min(
          math.max(new_mwfact, 0.01),
          0.99
        )
...
local function twopane_group(gs, cls, wa, orientation, fact, group, useless_gap)
  ...
  for c = group.first, group.last do
    ...
    geom[y] = coord
    gs[cls[c]] = geom
    hints.width, hints.height = apply_size_hints(
      cls[c],
      geom.width,
      geom.height,
      useless_gap
...
local function do_twopane(param, orientation)
  ...
  for _ = 1, 2 do
    if place_master and nmaster > 0 then
      ...
      coord = coord
        + twopane_group(gs, cls, wa, orientation, data[0], {
          first = 1,
          last = nmaster,
          coord = coord,
          size = size,
    ...
    if not place_master and nother > 0 then
      ...
      for i = 1, ncol do
        ...
        other_coord = coord
        coord = coord
          + twopane_group(gs, cls, wa, orientation, data[i], {
            first = first,
            last = last,
            coord = coord,
            size = size,
